        ...
        Object.getOwnPropertyNames(resObj).forEach(function(venueId, index, array) {
            ...
            if (venue.events && venue.events.data.length > 0) {
                ...
                venue.events.data.forEach(function(event, index, array) {
                    ...
                    //check if event has the venue as admin or owner
                    var is_admin = false,
                        has_page_admin = false;
                    var adminId;

                    if (event.owner && event.owner.id === venueId) {
                        is_admin = true;
                        has_page_admin = true;
                    } else if (event.admins) {
                        for (var i = 0, l = event.admins.data.length; i < l; i++) {
                            adminId = event.admins.data[i].id;
                            if (adminId === venueId) {
                                is_admin = true;
                                has_page_admin = true;
                            } else
                    ...
                    //event as only a user as admin
                    if (!is_admin && !has_page_admin && !event.is_page_owned) {
                        is_admin = true;
                    }
                    if (is_admin) {
                        var eventResultObj = {};
                        ...
                        if (!has_page_admin && !event.is_page_owned) {
                            eventResultObj.venue.id = event.owner.id;
                            eventResultObj.venue.name = event.owner.name;
                            eventResultObj.venue.about = null;
                            eventResultObj.venue.categories = null;
                            eventResultObj.venue.link = null;
                            eventResultObj.venue.username = null;
                            eventResultObj.venue.emails = null;
                            eventResultObj.venue.coverPicture = null;
                            eventResultObj.venue.profilePicture = null;
                        } else {
                        ...
                        if (event.place && event.place.location && event.place.location.latitude) {
                            eventResultObj.venue.location = event.place.location;
                        } else {
