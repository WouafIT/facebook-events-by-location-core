    ...
    return new Promise(function (resolve, reject) {
        ...
        if (!self.latitude || !self.longitude) {
            ...
            var idLimit = 50, //FB only allows 50 ids per /?ids= call
                ...
                venuesWithEvents = 0,
                eventsCount = 0,
                placeUrl = "https://graph.facebook.com/" + self.version + "/search?type=place&q=" + self.query + "&center=" + self.latitude + "," + self.longitude + "&distance=" + self.distance + "&limit=1000&fields=id&access_token=" + self.accessToken;
            ...
            rp.get(placeUrl).then(function(responseBody) {
                ...
                ids.forEach(function(idArray, index, arr) {
                    urls.push(rp.get("https://graph.facebook.com/" + self.version + "/?ids=" + idArray.join(",") + "&fields=id,name,about,emails,link,username,category_list,cover.fields(id,source),picture.type(large),location,events.fields(id,type,name,cover.fields(id,source),picture.type(large),description,start_time,end_time,attending_count,declined_count,maybe_count,noreply_count).since(" + currentTimestamp + ")&access_token=" + self.accessToken));
                });
