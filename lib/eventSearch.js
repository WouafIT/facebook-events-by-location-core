        ...
        if (!self.latitude || !self.longitude) {
            ...
            rp.get(placeUrl).then(function(responseBody) {
                ...
                ids.forEach(function(idArray, index, arr) {
                    urls.push(rp.get("https://graph.facebook.com/" + self.version + "/?ids=" + idArray.join(",") + "&fields=id,name,about,emails,link,username,mcover.fields(id,source),picture.type(large),location,events.fields(id,type,name,cover.fields(id,source),picture.type(large),description,start_time,end_time,attending_count,declined_count,maybe_count,noreply_count).since(" + currentTimestamp + ")&access_token=" + self.accessToken));
                });
